		...
		'onSuccess':function(json,text) {
			if(json.error) {
				...
				for(var id in json.price.products) {
				    ...
				    <?php } ?>
				    document.id('subtotal_discount_'+id) && document.id('subtotal_discount_'+id).set('text',json.price.products[id].subtotal_discount);
				    document.id('subtotal_with_tax_'+id) && document.id('subtotal_with_tax_'+id).set('html',json.price.products[id].subtotal_with_tax);
				}
	...
	?>
		...
		<div class="checkout-button-top">
			...
			if($userFieldsModel->getIfRequired('agreed')){
			    ?>
			    <label for ="tosAccepted" class="none">
			    <?php
				if(!class_exists('VmHtml'))require(JPATH_VM_ADMINISTRATOR.DS.'helpers'.DS.'html.php');
				echo VmHtml::checkbox('tosAccepted',$this->cart->tosAccepted,1,0,'class="terms-of-service" checked');
