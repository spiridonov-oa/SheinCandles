...
class plgSystemSourcererHelper
{
		...
		if ($docType == 'pdf') {
			...
			if (is_array($buffer)) {
				if (isset($buffer['component']) && isset($buffer['component'][''])) {
					$this->replaceInTheRest($buffer['component']['']);
				}
			} else {
	...
	{
		NNFrameworkFunctions::unprotectForm($str, array($this->src_params->syntax_start, $this->src_params->syntax_start_0, $this->src_params->syntax_end));
	}
