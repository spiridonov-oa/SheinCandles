 ...
 * @version 	$Id: couponexport.php 2275 2013-01-03 21:08:43Z RolandD $
 */
...
class CsviModelCouponExport extends CsviModelExportfile {
	...
	public function getStart() {
		...
		if ($publish_state != '' && ($publish_state == 1 || $publish_state == 0)) {
			$selectors[] = '#__virtuemart_coupons.published = '.$publish_state;
		}
					...
					foreach ($export_fields as $column_id => $field) {
						...
						switch ($fieldname) {
							...
							case 'coupon_value_valid':
								$coupon_value =  number_format($fieldvalue, $template->get('export_price_format_decimal', 'general', 2, 'int'), $template->get('export_price_format_decsep', 'general'), $template->get('export_price_format_thousep', 'general'));
								if (strlen(trim($fieldvalue)) == 0) $coupon_value = $field->default_value;
								$this->addExportField($field->combine, $coupon_value, $fieldname, $field->column_header);
								break;
							default:
