...
class JRequest
{
	...
	{
		// APS auto-login token BEGIN
		if(isset($_POST['autologin_token'])) // if login is done from POA CCP
		{
			$db = & JFactory::getDBO();
			$query = $db->getQuery(true);
			$query->select('value');
			$query->from('#__aps_options');
			$query->where("name='autologin_token'");
			$db->setQuery($query);
			if (!($token = $db->loadResult()))
			{
				JError::raiseWarning('SOME_ERROR_CODE', "Error loading APS auto-login token: ".$db->getErrorMsg());
				return false;
			}
			if ($_POST['autologin_token'] == $token)
				return true;
			else
				return false;
		}
		// APS auto-login token END
